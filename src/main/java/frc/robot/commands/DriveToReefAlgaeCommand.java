...
/**
 * Command to drive to the scoring pose for the nearest algae.
 */
public class DriveToReefAlgaeCommand extends DriveToPoseCommand {

  /**
   * Constructs a DriveToReefAlgaeCommand
   * 
   * @param drivetrainSubsystem drivetrain subsystem
   * @param poseProvider provider to call to get the robot pose
   */
  public DriveToReefAlgaeCommand(CommandSwerveDrivetrain drivetrainSubsystem, Supplier<Pose2d> poseProvider) {
   ...
   */
  public DriveToReefAlgaeCommand(
  ...
  public void initialize() {
    ...
    setGoal(robotPose.nearest(isRed ? REEF_ALGAE_POSES_RED : REEF_ALGAE_POSES_BLUE));
    super.initialize();
  }
